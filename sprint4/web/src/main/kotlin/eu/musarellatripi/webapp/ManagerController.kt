...
class ManagerController {
    ...
    fun homePage(model: Model): String {
        model.addAttribute("TMAX", Values.TMAX)
        return "manager"
    }
    ...
    @ResponseBody
    fun setTrolleyState(@PathVariable(value="state") state: String, request: HttpServletRequest): String {
        logger.info("${request.remoteAddr} ${request.method} ${request.requestURI} state=$state")
        val coapClient = CoapTalker("coap://localhost:8050/ctxservice/managerservice")

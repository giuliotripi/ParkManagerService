...
class Managerservice ( name: String, scope: CoroutineScope  ) : ActorBasicFsm( name, scope ){
	...
	override fun getBody() : (ActorBasicFsm.() -> Unit){
				...
				val Fan = org.eclipse.californium.core.CoapClient("coap://sensor.local:8054/ctxsensor/fan")
		...
		return { //this:ActionBasciFsm
				...
				state("idle") { //this:State
					action { //it:State
					 ...
					 transition(edgeName="t128",targetState="handleStopTrolley1",cond=whenRequest("stopTrolley"))
					transition(edgeName="t129",targetState="handleResumeTrolley",cond=whenRequest("resumeTrolley"))
					transition(edgeName="t130",targetState="updateSensorReading",cond=whenReply("sensorValue"))
					transition(edgeName="t131",targetState="dtFreeTimeout",cond=whenEvent("timeout"))
					transition(edgeName="t132",targetState="resetAlarmOutdoor",cond=whenEvent("carLeftOutdoor"))
					transition(edgeName="t133",targetState="update",cond=whenRequest("readStatus"))
					transition(edgeName="t134",targetState="handleTrolleyError",cond=whenDispatch("trolleyError"))
				}	 
				state("handleStopTrolley1") { //this:State
					action { //it:State
						request("getValue", "getValue(_)" ,"thermometer" )  
					}
					 transition(edgeName="t135",targetState="handleStopTrolley2",cond=whenReply("sensorValue"))
				}	 
				state("handleStopTrolley2") { //this:State
					action { //it:State
						if( checkMsgContent( Term.createTerm("sensorValue(VALUE,SENSORNAME)"), Term.createTerm("sensorValue(TEMPERATURE,temperature)"), 
												...
												val temperature = payloadArg(0).toDouble()
				...
				state("handleTrolleyError") { //this:State
					action { //it:State

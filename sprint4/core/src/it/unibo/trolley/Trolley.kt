		...
		return { //this:ActionBasciFsm
				state("s0") { //this:State
					action { //it:State
						...
						solve("simulateTrolley(X)","") //set resVar	
						println(currentSolution)
						 val x = getCurSol("X").toString() 
								   simulate = ( x == "on")
						println("[TROLLEY] simulate=$simulate")
					}
				...
				state("executeMove") { //this:State
					action { //it:State
					 ...
					 transition( edgeName="goto",targetState="waitStepResponse", cond=doswitchGuarded({ MV == "w" && !simulate  
					}) )
					transition( edgeName="goto",targetState="moving", cond=doswitchGuarded({! ( MV == "w" && !simulate  
					) }) )
				}	 
